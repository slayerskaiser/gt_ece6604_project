...
function fitBER = sim_flatRayleigh_PSK(EbNo,numPackets,MPSK)
...
%% Modulation
hMod = comm.PSKModulator(MPSK, 'PhaseOffset', 0, ...
    'BitInput', true);
hDemod = comm.PSKDemodulator(MPSK, 'PhaseOffset', 0, ...
    'BitOutput', true);

hAWGN1Rx = comm.AWGNChannel('NoiseMethod','Signal to noise ratio (Eb/No)', ...
    'SignalPower',1,'BitsPerSymbol',MPSK);
...
for idx = 1:length(EbNo)
    ...
    for packetIdx = 1:numPackets
        data = randi([0 1],frameLen*log2(MPSK),1);
